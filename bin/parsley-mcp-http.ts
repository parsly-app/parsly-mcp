...
const mcpPort = parseInt(process.env["PARSLEY_MCP_PORT"] ?? "9270", 10);
const bridgePort = parseInt(process.env["PARSLEY_PORT"] ?? "9271", 10);
...
httpServer.listen(mcpPort, "127.0.0.1", () => {
  console.error(`[parsley-mcp] Listening on http://127.0.0.1:${mcpPort}/mcp`);
  console.error(`[parsley-mcp] Bridge WebSocket on ws://127.0.0.1:${bridgePort}`);
});
